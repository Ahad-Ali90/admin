...
<x-admin.layouts.app>
  <div class="container-xxl py-3">
    {{-- Header --}}
    <div class="mb-4">
      <h1 class="h3 mb-1">Edit Booking</h1>
      <p class="text-secondary mb-0">Update details for {{ $booking->booking_reference }}.</p>
    </div>

    <form method="POST" action="{{ route('admin.bookings.update', $booking) }}" class="vstack gap-4">
      @csrf
      @method('PUT')
          ...
          <div class="row g-3">
            ...
            <div class="col-12 col-md-4">
              ...
              <input type="number" name="estimated_hours" id="estimated_hours" min="1"
                     value="{{ old('estimated_hours', $booking->estimated_hours) }}"
        ...
        <div class="card-body">
          ...
          <div class="row g-3">
            ...
            <div class="col-12">
              ...
              <textarea name="pickup_address" id="pickup_address" rows="3" class="form-control @error('pickup_address') is-invalid @enderror" required>{{ old('pickup_address', $booking->pickup_address) }}</textarea>
            ...
            <div class="col-12">
              ...
              <textarea name="delivery_address" id="delivery_address" rows="3" class="form-control @error('delivery_address') is-invalid @enderror" required>{{ old('delivery_address', $booking->delivery_address) }}</textarea>
            ...
            <div class="col-12 col-md-6">
              ...
              <input type="text" name="delivery_postcode" id="delivery_postcode"
                     value="{{ old('delivery_postcode', $booking->delivery_postcode) }}"
          ...
          <div class="row g-3">
            <div class="col-12">
              ...
              <textarea name="job_description" id="job_description" rows="4"
                        ...
                        placeholder="Describe what needs to be moved or delivered..." required>{{ old('job_description', $booking->job_description) }}</textarea>
            ...
            <div class="col-12">
              ...
              <textarea name="special_instructions" id="special_instructions" rows="3"
                        ...
                        placeholder="Any special requirements or instructions...">{{ old('special_instructions', $booking->special_instructions) }}</textarea>
            ...
            <div class="col-12 col-md-4">
              ...
              <select name="driver_id" id="driver_id" class="form-select @error('driver_id') is-invalid @enderror">
                ...
                @foreach($drivers as $driver)
                  <option value="{{ $driver->id }}" @selected(old('driver_id', $booking->driver_id)==$driver->id)>{{ $driver->name }}</option>
                @endforeach
              </select>
            ...
            <div class="col-12 col-md-4">
              ...
              <select name="porter_ids[]" id="porter_ids" class="form-select @error('porter_ids') is-invalid @enderror" multiple>
                @foreach($porters as $porter)
                  <option value="{{ $porter->id }}" @selected(in_array($porter->id, old('porter_ids', $booking->porter_ids ?? [])))>
                    {{ $porter->name }}
                  </option>
                @endforeach
              </select>
              ...
              <div class="service-row border rounded p-2">
                <div class="row g-2 align-items-end">
                  <div class="col-12 col-lg">
                    ...
                    <select class="form-select service-select">
                      ...
                      @foreach($services as $serviceType)
                        <option value="{{ $serviceType->id }}" @selected($service->id == $serviceType->id)>
                          {{ $serviceType->name }}
                        </option>
                      @endforeach
                    </select>
                  ...
                  <div class="col-6 col-md-2">
                    ...
                    <input type="number" min="1" step="1" class="form-control service-qty" value="{{ $service->pivot->quantity }}">
                  </div>
                ...
                {{-- hidden real inputs --}}
                <input type="hidden" class="real-service-id" name="services[{{ $index }}][service_id]" value="{{ $service->id }}">
                <input type="hidden" class="real-service-qty" name="services[{{ $index }}][qty]" value="{{ $service->pivot->quantity }}">
              </div>
            @endforeach
          </div>
      ...
      <div class="card">
        <div class="card-body">
          ...
          <div class="row g-3">
            <div class="col-12 col-md-6">
              ...
              <input type="number" step="0.01" min="0" id="manual_amount" name="manual_amount" 
                     ...
                     value="{{ old('manual_amount', $booking->manual_amount) }}" required>
            ...
            <div class="col-12 col-md-6">
              <div class="form-check">
                <input class="form-check-input" type="checkbox" id="is_company_booking" name="is_company_booking" value="1" 
                       @checked(old('is_company_booking', $booking->is_company_booking))>
          ...
          {{-- Company Commission --}}
          <div id="company-commission-section" class="row g-3 mt-3" style="display: {{ old('is_company_booking', $booking->is_company_booking) ? 'block' : 'none' }};">
            <div class="col-12 col-md-6">
              ...
              <input type="number" step="0.01" min="0" max="100" id="company_commission_rate" 
                     ...
                     value="{{ old('company_commission_rate', $booking->company_commission_rate) }}">
            </div>
          </div>
          ...
          <div class="row g-3 mt-3">
            <div class="col-12 col-md-4">
              ...
              <input type="number" min="0" id="extra_hours" name="extra_hours" 
                     class="form-control" value="{{ old('extra_hours', $booking->extra_hours) }}">
            </div>
            <div class="col-12 col-md-4">
              ...
              <input type="number" step="0.01" min="0" id="extra_hours_rate" name="extra_hours_rate" 
                     class="form-control" value="{{ old('extra_hours_rate', $booking->extra_hours_rate) }}">
            </div>
            <div class="col-12 col-md-4">
              <label class="form-label">Extra Hours Total</label>
              <input type="text" id="extra_hours_total" class="form-control" readonly value="{{ number_format($booking->extra_hours_amount, 2) }}">
            </div>
          </div>
        </div>
      </div>
  ...
  (function(){
    // Service management
    let serviceIndex = {{ $booking->services->count() }};
